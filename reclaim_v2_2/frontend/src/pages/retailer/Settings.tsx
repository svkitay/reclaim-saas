...
export default function Settings() {
  ...
  const handleSave = async () => {
    ...
    setSaved(false);
    try {
      // Only send non-empty values to avoid clearing existing keys
      const payload: any = {};
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-6 mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Sender Email" field="sender_email" type="email" placeholder="hello@ashleyfurniture.com" hint="Emails will appear to come from this address" />
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-6 mb-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">Required to send real emails to your customers</p>
          </div>
          <span className={`text-xs px-2 py-1 rounded-full font-medium ${user?.has_brevo ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}>
            {user?.has_brevo ? "✓ Connected" : "Not connected"}
          </span>
        </div>
        <div className="space-y-4">
          <Field
            ...
            hint="Get your free API key at brevo.com → Settings → API Keys"
          />
        </div>
        {!user?.has_brevo && (
          <div className="mt-4 p-3 rounded-lg bg-sky-50 border border-sky-200 text-sm text-sky-700">
            Without a Brevo key, Reclaim runs in demo mode — messages are generated and logged but not sent.{" "}
            <a href="https://app.brevo.com/settings/keys/api" target="_blank" rel="noopener noreferrer" className="underline font-medium">
              Get a free Brevo key →
            </a>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-6 mb-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">Optional — enables AI-personalized messages instead of templates</p>
          </div>
